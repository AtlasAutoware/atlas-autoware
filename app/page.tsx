    ...
    <div className="min-h-screen flex flex-col bg-[var(--bg-base)]">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 container mx-auto px-6 text-center pt-20">
          ...
          <h1
            className="text-5xl md:text-7xl lg:text-8xl font-bold leading-[1.08] mb-6 animate-slide-up text-gray-900 dark:text-white"
            style={{ fontFamily: 'var(--font-space)' }}
          >
            Driving the Future<br />
            <span className="gradient-text">of Autonomous Tech</span>
          </h1>
          <p className="text-lg md:text-xl text-slate-600 dark:text-slate-400 max-w-2xl mx-auto mb-10 leading-relaxed animate-slide-up delay-200">
            The premier high school-led self-driving car team — competing head-to-head with university teams from around the world.
          </p>
      ...
      {/* Stats bar */}
      <section className="py-14 border-y border-black/[0.06] dark:border-white/[0.06] bg-[var(--bg-surface)]">
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
            {stats.map((s) => (
              <div key={s.label}>
                <p className="text-3xl md:text-4xl font-bold gradient-text mb-1" style={{ fontFamily: 'var(--font-space)' }}>{s.value}</p>
      ...
      <section className="py-28">
        <div className="container mx-auto px-6">
          ...
          <p className="section-subtitle">
            We&apos;re not just building self-driving cars — we&apos;re shaping the future of transportation and inspiring a new generation of engineers.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {pillars.map((p, i) => {
              const Icon = p.icon
              return (
                <div key={p.title} className={`glass-card rounded-2xl p-8 group animate-slide-up delay-${(i + 1) * 100}`}>
                  <div className="w-12 h-12 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-6 group-hover:bg-red-500/20 group-hover:border-red-500/40 transition-all">
                    <Icon size={22} className="text-red-500 dark:text-red-400" />
                  </div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-3" style={{ fontFamily: 'var(--font-space)' }}>{p.title}</h3>
                  <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-sm">{p.description}</p>
                </div>
              )
            })}
          </div>
        ...
        <div className="container mx-auto px-6">
          <div className="glass-card rounded-2xl p-10 md:p-16 flex flex-col md:flex-row items-center gap-10 relative overflow-hidden">
            ...
            <div className="flex-shrink-0 w-20 h-20 rounded-2xl bg-red-500/10 border border-red-500/20 flex items-center justify-center">
              <Trophy size={36} className="text-red-500 dark:text-red-400" />
            </div>
            <div className="text-center md:text-left">
              <p className="text-xs font-semibold text-red-600 dark:text-red-400 uppercase tracking-widest mb-2">Competition</p>
              <h3 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white mb-3" style={{ fontFamily: 'var(--font-space)' }}>
                F1 Tenth — The World Stage
              </h3>
              <p className="text-slate-600 dark:text-slate-400 leading-relaxed max-w-xl">
                Atlas Autoware is the first and only high school team to compete in F1 Tenth, going head-to-head with university teams from around the globe in high-speed autonomous racing.
              </p>
            </div>
            <div className="md:ml-auto shrink-0">
              <Link href="/about" className="btn-primary whitespace-nowrap">Our Story <ArrowRight size={16} /></Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24">
        <div className="container mx-auto px-6 text-center">
          ...
          <p className="section-subtitle">
            We&apos;re looking for forward-thinking organisations to support our mission and help shape the future of autonomous vehicle technology.
          </p>
          <div className="glass-card rounded-2xl p-12 max-w-2xl mx-auto relative overflow-hidden">
            ...
            <p className="text-slate-600 dark:text-slate-400 mb-8 leading-relaxed relative z-10">
              Sponsoring Atlas Autoware puts your brand in front of a passionate, technically skilled community — and directly fuels the next generation of autonomous vehicle engineers.
            </p>
            <div className="flex flex-wrap gap-4 justify-center relative z-10">
              <a href="mailto:contact@atlasautoware.org" className="btn-primary">Become a Sponsor</a>
              <Link href="/sponsorship" className="btn-outline">View Tiers</Link>
            </div>
          </div>
        </div>
      </section>
